...
<!--添加修改弹出窗口-->
      <Form :model="formModel.fields" ref="formNotice" :rules="formModel.rules" label-position="top">
        <Row :gutter="16">
          ...
          <Col span="12">
            <FormItem label="价格" prop="price">
              <Input number v-model="formModel.fields.price" placeholder="药材价格" />
            </FormItem>
          </Col>
        </Row>
        ...
        <Row :gutter="16">
          <Col span="12">
            <FormItem label="库存" prop="stock">
              <Input number v-model="formModel.fields.stock" placeholder="库存数量" />
            </FormItem>
          </Col>
          <Col span="12">
        ...
        <FormItem class="notice"  label="介绍内容" label-position="top">
         <Input type="textarea" :maxlength="10" v-model="formModel.fields.detail" :rows="4" placeholder="介绍内容(最多10个字)" />
        </FormItem>
    ...
    >
      ...
      <Row :gutter="16" style="margin-top:20px;">
          ...
          <Col span="12">
            <FormItem label="价格" prop="price">
              <Input number  v-model="variantModel.fields.price" placeholder="收费价格" />
            </FormItem>
          </Col>
        </Row>
      ...
      <div class="demo-drawer-footer">
        <Button icon="md-checkmark-circle" type="primary" @click="handleSubmitUser2" >添 加</Button>
...
export default {
  ...
  methods: {
    ...
    handleEdit2(row) {
     ...
     this.variantModel.fields.productId=row.id;
     this.$refs["formNotice2"].resetFields();
      //获取列表数据信息
      var params={
        pageNum:this.stores.variant.query.pageNum,
        pageSize:this.stores.variant.query.pageSize,
        productId:row.id
      };
      listByProduct(params).then(res=>{
        if(res.data.status==200){
          this.stores.variant.data=res.data.data.list;
          this.stores.variant.query.totalCount=res.data.data.total;
        }
      });
    },
    delete_variant(row){
      deleteVariant({id:row.id}).then(res=>{
        if(res.data.status==200){
          var list=res.data.data;
          this.stores.variant.data=list;
    ...
    handleSubmitUser2(){
      addVariant(this.variantModel.fields).then(res=>{
        if(res.data.status==200){
          var list=res.data.data;
          this.stores.variant.data=list;
          this.$refs["formNotice2"].resetFields();
