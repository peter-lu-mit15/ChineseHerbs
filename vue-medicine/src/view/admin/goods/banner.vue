  ...
  <div>
    ...
    >
      ...
      >
        <Row :gutter="16">
          <FormItem label="标题" >
            <Input v-model="formModel.fields.BannerName" placeholder="请输入Banner标题" />
          </FormItem>
        </Row>

        <FormItem class="notice"  label="Banner图片" label-position="top">
...
export default {
  ...
  computed: {
    ...
    selectedRowsId() {
      return this.formModel.selection.map(x => x.bannerUuid);
    }
  },
  methods: {
    delImg(){
    ...
    loadconsumableList() {
      getBannerList(this.stores.consumable.query).then(res => {
    ...
    handleEdit(row) {
      ...
      this.doLoadConsumable(row.bannerUuid);
    },
    ...
    handleShowCreateWindow() {
      ...
      this.handleOpenFormWindow();
      this.handleResetFormConsumable();
      this.formModel.fields.BannerName="";
      this.img="";
      this.imgLen=0;
    },
    ...
    doCreateConsumable() {
      this.formModel.fields.Img=this.goodsimg;
      createBanner(this.formModel.fields).then(res => {
        if (res.data.code === 200) {
    ...
    doEditConsumable() {
      this.formModel.fields.Img=this.goodsimg;
      editBanner(this.formModel.fields).then(res => {
        if (res.data.code === 200) {
          this.$Message.success(res.data.message);
          this.handleCloseFormWindow();
          this.loadconsumableList();
    ...
    doLoadConsumable(Uuid) {
      loadOneBanner({ guid: Uuid }).then(res => {
        console.log(res.data.data);
        this.formModel.fields = res.data.data;
        this.formModel.fields.BannerName=res.data.data.bannerName;
        this.img=this.url+res.data.data.img;
        this.goodsimg=res.data.data.img;
